    ...
    try {
      const allUsersRef = collection(db, "users");
        ...
        const querySnap = await getDocs(q);
          const filterDocs = querySnap.docs.filter((doc) =>{
            return doc.id === auth.currentUser.uid
          })

          const newData = filterDocs[0]._document.data.value.mapValue.fields
          setFormData({
            ...formData,
            phoneNumber: newData.phoneNumber.stringValue,
            photoURL: newData.photoURL.stringValue
          })
        ...
        <div className="profileCard">
          <form>
            ...
            <input
              ...
              name="name"
              id="name"
              className={!updateDets ? "profileName" : "profileNameActive"}
              value={name}
              onChange={onChange}
              disabled={!updateDets}
            />
            <input
              ...
              id="phoneNumber"
              className={!updateDets ? "profileName" : "profileNameActive"}
              value={phoneNumber || auth.currentUser.phoneNumber || ""}
              onChange={onChange}
              disabled={!updateDets}
